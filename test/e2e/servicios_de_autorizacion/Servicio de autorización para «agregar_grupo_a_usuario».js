module.exports = async function (utilidades_de_test) {
    try {
        ...
        subtest("Servicio de autorización para «agregar_grupo_a_usuario» exige «id_grupo» e «id_usuario»", async function () {
            ...
            const token_de_sesion = utilidades_de_test.obtener_dato("token_de_sesion_de_administrador");
            const respuesta_1 = await axios.post(ruta_de_app("/auth/agregar_grupo_a_usuario"), { token_de_sesion });
            const respuesta_2 = await axios.post(ruta_de_app("/auth/agregar_grupo_a_usuario"), { token_de_sesion, id_usuario });
            const respuesta_3 = await axios.post(ruta_de_app("/auth/agregar_grupo_a_usuario"), { token_de_sesion, id_grupo });
            console.log(respuesta_1.data);
            console.log(respuesta_2.data);
        ...
        subtest("Servicio de autorización para «agregar_grupo_a_usuario» al final acepta los parámetros correctos", async function () {
            const { id_grupo, id_usuario } = metadatos;
            const token_de_sesion = utilidades_de_test.obtener_dato("token_de_sesion_de_administrador");
            const respuesta_1 = await axios.post(ruta_de_app("/auth/agregar_grupo_a_usuario"), { token_de_sesion, id_grupo, id_usuario });
            console.log(respuesta_1.data);
             revisor_de_objeto(respuesta_1, ["data", "respuesta"], respuesta => !("error" in respuesta));
        });
