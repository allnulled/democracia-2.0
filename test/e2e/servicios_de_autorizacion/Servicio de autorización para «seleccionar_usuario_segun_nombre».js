module.exports = async function (utilidades_de_test) {
    try {
        ...
        subtest("Servicio de autorización para «seleccionar_usuario_segun_nombre» exige un «nombre»", async function () {
        ...
        subtest("Servicio de autorización para «seleccionar_usuario_segun_nombre» al final acepta los parámetros correctos", async function () {
            const respuesta_1 = await axios.post(ruta_de_app("/auth/seleccionar_usuario_segun_nombre"), { nombre: "administrador" });
            console.log(respuesta_1.data);
            revisor_de_objeto(respuesta_1, ["data", "respuesta"], respuesta => !("error" in respuesta));
        });
