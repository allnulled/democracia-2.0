module.exports = async function (utilidades_de_test) {
    try {
        ...
        const { expect } = chai;
        const { subtest, iniciar } = tester("Conjunto de tests del servicio de datos para «insertar_item»", { debuga: 0 });
        const metadatos = {};

        subtest("Servicio de datos para «insertar_item» al final acepta los parámetros correctos", async function () {
            return "OK";
